...
from util import simple_sql_parser
...
class Node(object):
    ...
    def KeepRelevantJoins(self, all_join_conds):
        ...
        def _KeepRelevantJoins(s):
            join_pat = re.compile(r"(\w+)\.(\w+)\s*(>=|<=|>|<|=)\s*(\w+)\.(\w+)", re.VERBOSE)
            split_join = join_pat.findall(s)

            # To check for specific interval patterns in STACK
            if len(split_join) == 0:
                interval_pat = re.compile(r"(\w+)\.(\w+)\s*(>=|<=|>|<|=)\s*(\w+)\.(\w+\s*\+\s*'\w+\s*\w+'::interval)", re.VERBOSE)
                split_join = interval_pat.findall(s)

                # if the order has been changed from a1.col >= a2.col + '1 year'::interval
                # to a2.col + '1 year'::interval <= a1.col
                #
                if len(split_join) == 0:
                    interval_pat = re.compile(f"(\w+)\.(\w+\s*\+\s*'\w+\s*\w+'::interval)\s*(>=|<=|>|<|=)\s*(\w+)\.(\w+)", re.VERBOSE)
                    split_join = interval_pat.findall(s)

            l_alias, _, _, r_alias, _ = split_join[0]
    ...
    def IsJoin(self):
        return 'Join' in self.node_type or self.node_type == 'Nested Loop' or self.node_type == 'NestLoop'
    ...
    def to_sql(self,
        ...
        if with_filters:
            ...
            interval_pat = re.compile(r"(\w+)\.(\w+)\s*(>=|<=|>|<|=)\s*(\w+)\.(\w+\s*\+\s*'\w+\s*\w+'::interval)", re.VERBOSE)
            interval_pat__reverse = re.compile(r"(\w+)\.(\w+\s*\+\s*'\w+\s*\w+'::interval)\s*(>=|<=|>|<|=)\s*(\w+)\.(\w+)", re.VERBOSE)
            
            for f in filters:
                f = f.strip()
                if 'interval' not in f:
                    continue

                split_join = interval_pat.findall(f)
                if len(split_join) > 0:
                    # type: a1.col1 >= a2.col2 + '1 year'::interval
                    l_alias, l_col, operation, r_alias, r_col = split_join[0]
                    if r_alias not in [x.split(' AS ')[1] for x in self.leaf_ids()]:
                        if r_alias == 'q1':
                            from_str += ', question AS q1'
                        elif r_alias == 'b1':
                            from_str += ', badge AS b1'

                else:
                    # type: a2.col2 + '1 year'::interval <= a1.col1 
                    split_join = interval_pat__reverse.findall(f)
                    l_alias, l_col, operation, r_alias, r_col = split_join[0]
                    if l_alias not in [x.split(' AS ')[1] for x in self.leaf_ids()]:
                        if l_alias == 'q1':
                            from_str += ', question AS q1'
                        elif l_alias == 'b1':
                            from_str += ', badge AS b1'
    ...
    def hint_str(self, with_physical_hints=True):
...
class QueryFeaturizer(Featurizer):
    ...
    def __call__(self, node):
        ...
        # 2023-08-02: Make sure that all nodes of all_aliases are in the
        # query_join_graph available, otherwise this function call will break
        for alias in all_aliases:
            if alias not in query_join_graph.nodes():
                query_join_graph.add_node(alias)
